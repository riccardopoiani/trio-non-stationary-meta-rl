...
from active_learning.observation_utils import get_posterior_no_prev, oracle_augment_obs
...
class DecoupledPMTAgent:
    ...
    def train(self, training_iter, env_name, seed, task_generator,
        ...
        for k in range(training_iter):
            ...
            self.multi_task_policy_step(envs, prior, new_tasks, use_env_obs)
    ...
    def vae_step(self, num_vae_steps, use_env_obs, task_generator, env_name, seed, log_dir, verbose):
        train_loss = 0
        mse_train_loss = 0
        kdl_train_loss = 0
        ...
        for vae_step in range(num_vae_steps):
            ...
            obs = envs.reset()
            obs = oracle_augment_obs(obs=obs, use_env_obs=use_env_obs, latent_dim=self.latent_dim,
                                     latent=prior[:, 0:self.latent_dim])
            ...
            for step in range(num_data_context):
                ...
                obs = oracle_augment_obs(obs=obs, latent_dim=self.latent_dim, use_env_obs=use_env_obs,
                                         latent=posterior[:, 0:self.latent_dim])
    ...
    def multi_task_policy_step(self, envs, prior, task_param, use_env_obs):
        # Multi-task learning with posterior mean
        obs = envs.reset()
        obs = oracle_augment_obs(obs=obs, latent=task_param, latent_dim=self.latent_dim,
                                 use_env_obs=use_env_obs)
        ...
        for step in range(self.num_steps):
            ...
            obs, reward, done, infos = envs.step(action)
            obs = oracle_augment_obs(obs, task_param, self.latent_dim, use_env_obs)
    ...
    def evaluate(self, num_task_to_evaluate, task_generator, log_dir, seed, use_env_obs, env_name):
        ...
        for _ in range(n_iter):
            ...
            prior = torch.tensor([p.flatten().tolist() for p in prior])
            ...
            while len(eval_episode_rewards) < self.num_processes:
                ...
                obs = oracle_augment_obs(obs=obs, latent_dim=self.latent_dim, latent=posterior[:, 0:self.latent_dim],
                                         use_env_obs=use_env_obs)
    ...
    def test_task_sequence(self, gp_list, sw_size, env_name, seed, log_dir, envs_kwargs_list, init_prior, use_env_obs,
        ...
        prior = torch.tensor([p.flatten().tolist() for p in init_prior])
        ...
        for t, kwargs in enumerate(envs_kwargs_list):
            ...
            while len(task_epi_rewards) < num_eval_processes:
                ...
                obs = oracle_augment_obs(obs=obs, latent=posterior[:, 0:self.latent_dim],
                                         use_env_obs=use_env_obs, latent_dim=self.latent_dim)
