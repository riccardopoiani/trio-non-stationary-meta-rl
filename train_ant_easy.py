...
from task.ant_task_generator_easy import AntTaskGeneratorEasy
...
def main():
    ...
    folder = "result/ant/"
    env_name = "antfrictionfull-v0"

    # Task family parameters
    friction_var_min = 0.0001
    ...
    task_generator = AntTaskGeneratorEasy(n_frictions=8,
                                          friction_var_min=friction_var_min,
                                          friction_var_max=friction_var_max)
    prior_std_max = [friction_var_max ** (1 / 2) for _ in range(latent_dim)]
    prior_std_min = [friction_var_min ** (1 / 2) for _ in range(latent_dim)]
    ...
    if args.algo == 'rl2':
        obs_shape = (111 + 8 + 1,)  # 2 obs_shape + 2 action_shape + 1 reward
