...
class AntTaskGeneratorAdvanced(TaskGenerator):
    ...
    def sample_rnd_leg_tasks(self, num_p):
        ...
        for p in range(num_p):
            # Rnd leg 1 - Mean
            max_m[p, rnd_leg_idx[p]] = self.max_mean_rnd
            min_m[p, rnd_leg_idx[p]] = self.min_mean_rnd

            # Rnd leg 1 - Std
            max_s[p, rnd_leg_idx[p]] = self.max_std_rnd
            min_s[p, rnd_leg_idx[p]] = self.min_std_rnd

            # Rnd leg 2 - Mean
            max_m[p, rnd_leg_idx_2[p]] = self.max_mean_rnd
            min_m[p, rnd_leg_idx_2[p]] = self.min_mean_rnd

            # Rnd leg 2 -Std
            max_s[p, rnd_leg_idx_2[p]] = self.max_std_rnd
            min_s[p, rnd_leg_idx_2[p]] = self.min_std_rnd
        ...
        for p in range(num_p):
            for leg in range(4):
                if leg != rnd_leg_idx[p] and leg != rnd_leg_idx_2[p]:
                    std[p, leg] = (self.max_std_rnd - self.min_std_rnd) / (self.max_std_ok - self.min_std_ok) * (std[p, leg] - self.max_std_ok) + self.max_std_rnd
    ...
    def sample_broke_leg_tasks(self, num_p):
        ...
        for p in range(num_p):
            # Rnd leg 1 - Mean
            max_m[p, broken_leg_idx[p]] = self.max_mean_broken
            min_m[p, broken_leg_idx[p]] = self.min_mean_broken

            # Rnd leg 2 - Mean
            max_m[p, broken_leg_idx_2[p]] = self.max_mean_broken
            min_m[p, broken_leg_idx_2[p]] = self.min_mean_broken
